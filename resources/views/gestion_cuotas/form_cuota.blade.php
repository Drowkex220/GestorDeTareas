...
@section('content')
    ...
    <form action="{{ route('saveCuota', ['modo' => $modo]) }}" method="POST" class="formulario form form-control">
        @csrf
        ...
        <select class="form-control" id="pagada" name="pagada">
            <option value="1" {{ (old('pagada', isset($cuota) ? $cuota->pagada : '') == '1') ? 'selected' : '' }}>Sí</option>
            <option value="0" {{ (old('pagada', isset($cuota) ? $cuota->pagada : '') == '0') ? 'selected' : '' }}>No</option>
        </select>
        ...
        <div class="data">
            ...
            <label for="id_tarea">Selecciona una tarea:</label>
